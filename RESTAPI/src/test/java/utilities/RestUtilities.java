...
public class RestUtilities {
	...
	public static Response getResponse(MethodType methodType, String payload)
	{
		...
		case GET: response = given().spec(REQ_SPECIFICATION).log().all(true).when().get(ENDPOINT);
		break;
		
		case PUT: response = given().spec(REQ_SPECIFICATION).when().put(ENDPOINT);
		break;
		
		case DELETE: response = given().spec(REQ_SPECIFICATION).when().delete(ENDPOINT);
		break;
		
		default: System.out.println("Method Type is not proper.");
		break;
